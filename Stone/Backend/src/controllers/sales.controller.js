...
// getAllSalesWithItems: async (req, res) => {
//   try {
//     const sales = await Sales.getAll();
...
const SalesController = {
  ...
  createSale: async (req, res) => {
    try {
      ...
      if (!customer_id || !bill_date || !items || !items.length) {
        return res.status(400).json({ error: 'Customer, date & items are required' });
      }
  ...
  getSaleByIdWithItems: async (req, res) => {
    try {
      const sale_id = req.params.id;
      const sale = await Sales.getById(sale_id);
      if (!sale) return res.status(404).json({ error: 'Sale not found' });
